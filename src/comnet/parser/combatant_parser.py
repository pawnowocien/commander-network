...
from .models import ParseCountry

# Works for simple cases
def parse_combatant(combatant_code: mwp.wikicode.Wikicode) -> list[ParseCountry]:
    countries = []
    for node in combatant_code.nodes:
        ...
        if isinstance(node, mwp.nodes.Template):
            if node.name.strip().lower() == "flagcountry":
                ...
                countries.append(ParseCountry(name=country_name))
        # Without flagicons
