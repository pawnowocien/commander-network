...
def _show_and_cut(G: nx.Graph, com_to_country: dict[str, str], country_to_col: dict[str, str], name: str) -> tuple[nx.Graph, dict]:
    ...
    save_graph_as_img(G, colors=get_com_to_col(com_to_country, country_to_col), output_file=f"{DIR_PATH}/{name}_full", weights=True, pos=pos)
    G_cut = remove_small_components(G, min_size=10)
    save_graph_as_img(G_cut, colors=get_com_to_col(com_to_country, country_to_col), output_file=f"{DIR_PATH}/{name}_cut", weights=True, pos=pos)
...
def _do_community_analysis(G: nx.Graph, com_to_country: dict[str, str], country_to_col: dict[str, str], pos: dict, name: str):
    scores = run_predictions(G, get_com_to_col(com_to_country, country_to_col), pos=pos, output_path=f"{DIR_PATH}/communities/{name}")
    save_communities_stats(scores, f"{DIR_PATH}/communities/{name}_stats.png") # TODO



def _do_triads_analysis(allies: set[tuple[str, str]], enemies: set[tuple[str, str]], com_to_country: dict[str, str]):
    os.makedirs(f"{DIR_PATH}/triads/", exist_ok=True)

    count, _ = open_triad_types(allies, enemies)
    save_triad_stats(count, f"{DIR_PATH}/triads/open_triad_stats.png")

    count, exceptions = closed_triad_types(allies, enemies)
    save_triad_stats(count, f"{DIR_PATH}/triads/closed_triad_stats.png")

    with open(f"{DIR_PATH}/triads/exceptions.txt", "w", encoding="utf-8") as f:
...
def _do_centrality_analysis(G: nx.Graph, com_to_country: dict[str, str], country_to_col: dict[str, str], name: str):
    ...
    for centrality, scores in centrality_scores.items():
        ...
        save_centrality_stats(list((d[1], d[0]) for d in sorted_scores),
                              com_to_country, country_to_col, f"{DIR_PATH}/centrality/{name}_{centrality}.png", name=centrality)
